...
def compute_all_distances(vesicles, ribbon, pd, resolution, save_folder):
    ...
    return ves_save, ribbon_save, pd_save


def visualize_distances(
    tomo, vesicles, ribbon, pd,
    distance_path_vesicles, distance_path_ribbon, distance_path_pd,
    scale=2, show=True,
):
    import napari
    ...
    if not show:
        return ves_dist, ribbon_dist, pd_dist
...
def to_excel(vesicle_distances, ribbon_distances, pd_distances, result_path):
...
def process_distance_measurements(tomo_path, show):
    ...
    ves_dist, ribbon_dist, pd_dist = compute_all_distances(vesicles, ribbon, pd, resolution, distance_save_folder)

    with open_file(tomo_path, "r") as f:
        tomo = f["data"][:]
    ves_dist, ribbon_dist, pd_dist = visualize_distances(
        tomo, vesicles, ribbon, pd, ves_dist, ribbon_dist, pd_dist, show=show
    )

    out_path = os.path.join(distance_save_folder, "distances.xlsx")
    to_excel(ves_dist, ribbon_dist, pd_dist, result_path=out_path)
